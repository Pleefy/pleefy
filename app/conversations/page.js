      ...
      <Card>
        <CardHeader className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="primary" onClick={exportXLSX}>Export Excel</Button>
          </div>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TBody>
              {data.map((c) => (
                <TR key={c.id}>
                  ...
                  <TD>
                    <div className="flex gap-2">
                      <Button variant={c.thumbs==="up"?"primary":"outline"} onClick={() => setThumb(c.id,"up")}>👍</Button>
                      <Button variant={c.thumbs==="down"?"primary":"outline"} onClick={() => setThumb(c.id,"down")}>👎</Button>
                    </div>
                  </TD>
                </TR>
              ))}
